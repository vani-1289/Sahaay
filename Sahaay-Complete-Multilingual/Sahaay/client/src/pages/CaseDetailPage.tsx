  ...
  if (!caseData) {
    return (
      <div className="soft-card p-10 text-center space-y-3">
        <h3 className="text-base font-bold text-[#123B5D]">{t('noParcelsFoundTitle', language)}</h3>
        <p className="text-xs text-[#667784]">{t('serviceTrackCaseDesc', language)}</p>
        <button
          onClick={() => navigate('/')}
    ...
    <div className="space-y-6">
      ...
      <div className="soft-card overflow-hidden">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <span className="text-[10px] uppercase font-bold tracking-wider text-[#E8B84A] block">
                {t('statutorySectionLabel', language)}
              </span>
              <h1 className="text-lg sm:text-xl font-black tracking-tight">
                {t('surveyParcelLabel', language)} #{caseData.caseReference}
              </h1>
            </div>
          </div>
        ...
        <div className="p-6 grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs bg-[#F8FAFC] border-b border-[#DDE6EC]">
          ...
          <div>
            <span className="text-[#667784] block font-semibold text-[11px]">{t('surveyParcelLabel', language)}:</span>
            <span className="font-mono font-bold text-[#243746] block mt-0.5">#{parcel?.surveyNumber}</span>
          </div>
          <div>
            <span className="text-[#667784] block font-semibold text-[11px]">{t('villageTehsilLabel', language)}:</span>
            <span className="font-semibold text-[#243746] block mt-0.5">{parcel?.village}, {parcel?.district}</span>
          </div>
          <div>
            <span className="text-[#667784] block font-semibold text-[11px]">{t('certifiedAreaLabel', language)}:</span>
            <span className="font-bold text-[#2E7D5B] block mt-0.5">{parcel?.recordedAreaHa} ha</span>
          </div>
        </div>
      ...
      <div className="flex border-b border-[#DDE6EC] bg-white rounded-xl p-1 shadow-soft overflow-x-auto gap-1 text-xs">
        ...
        >
          {t('documents', language)} ({docs.length})
        </button>
        ...
        >
          {t('actions', language)} ({actions.length})
        </button>
      </div>
        ...
        <div className="soft-card p-6 sm:p-8 space-y-6">
          <div className="flex items-center justify-between border-b border-[#DDE6EC] pb-3">
            ...
            <span className="text-xs bg-[#EAF3F8] text-[#123B5D] font-bold px-3 py-1 rounded-md border border-[#DDE6EC]">
              {t('currentStageLabel', language)}: {timelineData.currentStage}
            </span>
          </div>
      ...
      {activeTab === 'rr' && rr && (
        <div className="soft-card p-6 sm:p-8 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-[#F8FAFC] border border-[#DDE6EC] space-y-1">
              <span className="text-[#667784] text-[11px] block uppercase font-semibold">{t('rrLivelihoodTitle', language)}:</span>
              <span className="text-base font-bold text-[#123B5D] font-mono">₹{rr.subsistenceGrantINR?.toLocaleString('en-IN') || '5,00,000'}</span>
            </div>
            <div className="p-4 rounded-xl bg-[#F8FAFC] border border-[#DDE6EC] space-y-1">
              <span className="text-[#667784] text-[11px] block uppercase font-semibold">{t('rrHouseTitle', language)}:</span>
              <span className="text-sm font-bold text-[#243746]">{rr.housingSupport || t('rrHouseDesc', language)}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'documents' && (
        <div className="soft-card p-6 sm:p-8 space-y-4">
          <h3 className="text-sm font-bold text-[#123B5D] border-b border-[#DDE6EC] pb-2">
            {t('officialDocumentsTitle', language)} ({docs.length})
          </h3>
      ...
      {activeTab === 'actions' && (
        <div className="soft-card p-6 sm:p-8 space-y-4">
          <h3 className="text-sm font-bold text-[#123B5D] border-b border-[#DDE6EC] pb-2">
            {t('actionCenterTitle', language)} ({actions.length})
          </h3>
