...
import { CheckSquare, Calendar, AlertCircle, CheckCircle2, FileText, ArrowRight, ShieldCheck } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="soft-card p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2.5">
            ...
            <h1 className="text-xl font-extrabold text-[#123B5D] tracking-tight">
              {t('actions', language)} • Statutory Action Checklist
            </h1>
          </div>
          <p className="text-xs sm:text-sm text-[#667784] mt-1">
            Mandatory citizen verification tasks, joint survey appointments, and response deadlines for your case.
          </p>
        </div>
        ...
        <span className="px-3 py-1.5 bg-[#EAF3F8] text-[#123B5D] text-xs font-mono font-bold rounded-lg border border-[#DDE6EC] self-start sm:self-auto">
          Case: {primaryCase?.caseReference || 'SHY-2024-MP-0914'}
        </span>
      </div>
      ...
      <div className="space-y-4">
        {actions.length > 0 ? (
                  ...
                  <div className="space-y-2 max-w-3xl">
                    ...
                    {act.deadline && (
                      <div className="flex items-center space-x-1.5 text-xs font-bold text-[#C62828] pl-10">
                        ...
                        <span>Statutory Submission Cut-off: {new Date(act.deadline).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                      </div>
                    )}
                  </div>
          ...
          <div className="soft-card p-10 text-center text-[#667784] space-y-2">
            ...
            <p className="text-xs font-bold text-[#123B5D]">All statutory checklist actions are currently up to date.</p>
          </div>
        )}
      </div>
    </div>
