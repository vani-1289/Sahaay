    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-xl border border-[#DDE6EC] shadow-soft flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2.5">
            ...
            <h1 className="text-xl font-extrabold text-[#123B5D] tracking-tight">
              Find My Land Records (भू-अभिलेख खोज)
            </h1>
          </div>
          <p className="text-xs sm:text-sm text-[#667784] mt-1">
            Search your land using survey/khasra numbers or district cadastral registers to verify acquisition status.
          </p>
        </div>
        ...
        >
          ...
          <span>Upload Notice to Auto-Match</span>
        </button>
      </div>
      ...
      <div className="soft-card p-6 space-y-4">
        ...
        {activeTab === 'survey' && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. 1042 or 1042/1"
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-[#667784] pt-1">
              <span className="font-semibold">Quick Demo Records:</span>
              <button
                type="button"
        ...
        {activeTab === 'qr' && (
          <div className="space-y-3 max-w-md">
            <label className="block text-xs font-semibold text-[#123B5D]">
              Statutory Case ID / Reference Number
            </label>
            <input
              type="text"
        ...
        {activeTab === 'gps' && (
          <div className="space-y-3 max-w-md text-xs text-[#667784]">
            <p>
              GPS centroid detected: <strong>23.2625° N, 77.4150° E</strong> (Rampur / Bhopal Circle).
            </p>
            <button
            ...
            >
              ...
              <span>View Map at Current Centroid</span>
            </button>
          </div>
        )}
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between border-b border-[#DDE6EC] pb-2">
          <h2 className="text-sm font-bold text-[#123B5D] uppercase tracking-wider">
            Matching Cadastral Land Records ({results.length})
          </h2>
        </div>
        ...
        {results.length > 0 ? (
                ...
                >
                      ...
                      <div>
                        <div className="flex items-center space-x-2">
                          <h3 className="text-base font-bold text-[#123B5D]">
                            Survey #{parcel.surveyNumber} {parcel.khasraNumber && `(Khasra ${parcel.khasraNumber})`}
                          </h3>
                          <StatusBadge status={parcel.currentStatus} />
                        </div>
                        <p className="text-xs text-[#667784] mt-0.5">
                          Village: <strong>{parcel.village}</strong> • Tehsil: <strong>{parcel.tehsil}</strong> • District: <strong>{parcel.district}</strong>
                        </p>
                      </div>
                    </div>

                    <div className="text-left sm:text-right text-xs">
                      <span className="text-[#667784] block">Recorded Area</span>
                  ...
                  {matchedCase ? (
                    <div className="bg-[#F5FAFC] border border-[#DDE6EC] rounded-xl p-4 text-xs space-y-3">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <span className="font-mono font-bold text-[#123B5D] bg-white px-2.5 py-1 rounded-md border border-[#DDE6EC]">
                          Case #{matchedCase.caseReference}
                        </span>
                      </div>

                      <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-[#DDE6EC]">
                        <span className="text-[#667784]">
                          Current Stage: <strong>{matchedCase.stage}</strong>
                        </span>
                        {matchedCase.estimatedCompensationINR > 0 && (
                          <span className="text-[#667784]">
                            Estimated Valuation: <strong className="text-[#2E7D5B]">₹{(matchedCase.estimatedCompensationINR).toLocaleString('en-IN')}</strong>
                          </span>
                        )}
                      </div>
                    ...
                    <p className="text-xs text-[#667784] bg-[#F8FAFC] p-3 rounded-lg border border-[#DDE6EC]">
                      No active acquisition proceeding registered for this parcel.
                    </p>
                  )}
                </div>
              );
          ...
          hasSearched && !loading && (
            <div className="bg-white border border-[#DDE6EC] rounded-xl p-8 text-center space-y-3">
              ...
              <h3 className="text-sm font-bold text-[#123B5D]">No Matching Land Records Found</h3>
              <p className="text-xs text-[#667784] max-w-sm mx-auto">
                No parcel was found with the specified parameters. Please search for survey <strong>1042</strong> or upload your paper notice.
              </p>
            </div>
          )
        )}
