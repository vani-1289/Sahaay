...
import { Bell, CheckCheck, Clock, FileText, AlertTriangle, ShieldCheck, CheckCircle2 } from 'lucide-react';
        ...
        <div>
          <div className="flex items-center space-x-2.5">
            ...
            <h1 className="text-xl font-extrabold text-[#123B5D] tracking-tight">
              {t('notifications', language)} • Official Notice Log
            </h1>
          </div>
          <p className="text-xs sm:text-sm text-[#667784] mt-1">
            Real-time updates on case milestones, officer grievance responses, and compensation releases.
          </p>
        </div>
      ...
      <div className="space-y-3">
        {notifications.length > 0 ? (
                ...
                <div className="space-y-1">
                  ...
                  <span className="text-[11px] text-[#667784] block pt-1">
                    {new Date(n.createdAt).toLocaleDateString('en-IN', {
                      ...
                      month: 'short',
                      year: 'numeric',
                    })} at {new Date(n.createdAt).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })} IST
                  </span>
                </div>
              </div>
          ...
          <div className="soft-card p-10 text-center text-[#667784] space-y-2">
            ...
            <p className="text-xs font-bold text-[#123B5D]">No alerts or notices currently in your log.</p>
          </div>
        )}
      </div>
    </div>
