        ...
        <div>
          <div className="flex items-center space-x-2.5">
            ...
            <h1 className="text-xl font-extrabold text-[#123B5D] tracking-tight">
              Cadastral GIS Map Portal (भू-स्थानिक मानचित्र)
            </h1>
          </div>
          <p className="text-xs sm:text-sm text-[#667784] mt-1">
            Cadastral boundary overlays, Khasra survey demarcation, and infrastructure right-of-way alignments.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 soft-card overflow-hidden h-[540px] relative">
            ...
            <Marker position={[23.2625, 77.4150]} icon={customMarkerIcon}>
              <Popup>
                <div className="p-1 text-xs">
                  <strong>Survey #{selectedParcel?.surveyNumber || '1042'}</strong>
                  <br />
          ...
          <div className="absolute bottom-4 left-4 z-[1000] bg-white/95 backdrop-blur-sm border border-[#DDE6EC] p-3 rounded-lg shadow-soft text-xs space-y-1.5">
            <div className="font-bold text-[#123B5D] text-[11px] uppercase tracking-wider">Map Legend</div>
            <div className="flex items-center space-x-2 text-[#243746]">
              <span className="w-3 h-3 rounded-sm bg-[#2E7D5B]"></span>
              <span>Selected Cadastral Plot</span>
            </div>
            <div className="flex items-center space-x-2 text-[#243746]">
          ...
          {selectedParcel ? (
            <div className="space-y-4 text-xs">
              ...
              {selectedParcel.cases?.[0] && (
                <div className="p-3.5 rounded-xl bg-[#FFF9F0] border border-[#E8B84A]/60 space-y-2">
                  <div className="font-bold text-[#123B5D] text-xs">
                    Active Acquisition: {selectedParcel.cases[0].project?.name}
                  </div>
                  <div className="text-[11px] text-[#667784]">
                    Case Ref: <strong>#{selectedParcel.cases[0].caseReference}</strong> • Stage: <strong>{selectedParcel.cases[0].stage}</strong>
                  </div>
                  <button
              ...
              >
                ...
                <span>File Joint Measurement Request</span>
              </button>
            </div>
          ) : (
            <p className="text-xs text-[#667784]">Select a parcel from the map to inspect records.</p>
          )}
        </div>
      </div>
    </div>
